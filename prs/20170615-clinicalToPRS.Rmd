...
```{r}
gf = read.csv('~/data/prs/clinical_06162017.csv')
gf = gf[gf$ADHD_current!='exclude',]
...
```{r}
# GONNA DO A QUICK HACK HERE BECAUSE WE'RE CURRENTLY MISSING MANY FAMILY IDS
df = df[!is.na(df$NuclearFamID), ]
...
```{r}
write.csv(all_res, file='~/philip/prs_results_06162017.csv')
```

# TODO:
* clean up clinical file
* add FAMIDs to clinical file
* play with age groups
* run mediation analysis for good correlations
* repeat analysis with european PGC sample only
