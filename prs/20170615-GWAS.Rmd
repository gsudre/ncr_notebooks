...
```{r}
genr_prs = read.csv('~/data/gwas_brain_growth/ADHD_new_GENR_SCORES_N5731.csv')
gwas = read.csv('~/data/gwas_brain_growth/FINAL_pheno_file_nhgri_genr_combined_18Aug2016.csv')
mgenr = merge(gwas, genr_prs, by.x='PERSON', by.y='IDC')
```
...
```{r}
library(nlme)
dep_vars = which(grepl('^Nresid', colnames(mgenr)))
