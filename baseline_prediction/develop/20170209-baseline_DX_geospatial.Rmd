...
Using the raw values and the PCA was about the same. Let's try adding SES to each one and see if they improve. But the issue is NANs in SES, so we'll need to impute or remove subjects:
...
ldata = ldata_orig
groups = groups_orig
root_fname = '~/tmp/GeoRawSES_meanImputed_NVvsADHD'
sink(sprintf('%s.log', root_fname), append=FALSE, split=TRUE)
...
root_fname = '~/tmp/GeoRawSES_NAremoved_NVvsADHD'
inTrain = createDataPartition(y = groups,
                              times= nsplits,
                              p=train_test_ratio)
source('~/ncr_notebooks/baseline_prediction/src/do_classification.R')
...
Now we do a PCA first, then add SES:
...
ldata_orig = data[, keep_me]
ldata.pca <- prcomp(ldata_orig, center=T, scale=T)
```
...
set.seed(107)
groups = groups_orig
ldata = cbind(ldata.pca$x[, 1:4], data$SES)
colnames(ldata)[5] = 'SES'
preProcValues <- preProcess(ldata, method = c("medianImpute"))
ldata <- predict(preProcValues, ldata)
root_fname = '~/tmp/GeoRawPCAEV1SES_medianImputed_NVvsADHD'
sink(sprintf('%s.log', root_fname), append=FALSE, split=TRUE)
run_models = c('rndForest', 'lr', 'rsvm', 'xgb') # c('rndForest', 'lr', 'lsvm', 'rsvm', 'xgb', 'gbm')
inTrain = createDataPartition(y = groups,
                              times= nsplits,
...
Same as above, but using the PCA elbow:

```{r}
set.seed(107)
ldata = cbind(ldata.pca$x[, 1:3], data$SES)
colnames(ldata)[4] = 'SES'
preProcValues <- preProcess(ldata, method = c("medianImpute"))
ldata <- predict(preProcValues, ldata)
root_fname = '~/tmp/GeoRawPCAElbowSES_medianImputed_NVvsADHD'
sink(sprintf('%s.log', root_fname), append=FALSE, split=TRUE)
run_models = c('rndForest', 'lr', 'rsvm', 'xgb') # c('rndForest', 'lr', 'lsvm', 'rsvm', 'xgb', 'gbm')
inTrain = createDataPartition(y = groups,
                              times= nsplits,
...
Now, we first remove NA subjects, then do the PCA:
...
ldata = as.data.frame(ldata[keep_me, ])
groups = groups[keep_me]
ldata.pca <- prcomp(ldata[1:13], center=T, scale=T)
plot(ldata.pca, type = "l")
print(ldata.pca$sdev^2)  # these are the eignevalues
```
...
```{r}
set.seed(107)
ldata = cbind(ldata.pca$x[, 1:3], ldata$SES)
colnames(ldata)[4] = 'SES'
root_fname = '~/tmp/GeoRawPCAElbowSES_NAremoved_NVvsADHD'
sink(sprintf('%s.log', root_fname), append=FALSE, split=TRUE)
run_models = c('rndForest', 'lr', 'rsvm', 'xgb') # c('rndForest', 'lr', 'lsvm', 'rsvm', 'xgb', 'gbm')
inTrain = createDataPartition(y = groups,
                              times= nsplits,
...
groups = groups[keep_me]
ldata.pca <- prcomp(ldata[1:13], center=T, scale=T)
ldata = cbind(ldata.pca$x[, 1:4], ldata$SES)
colnames(ldata)[5] = 'SES'
root_fname = '~/tmp/GeoRawPCAEV1SES_NAremoved_NVvsADHD'
sink(sprintf('%s.log', root_fname), append=FALSE, split=TRUE)
run_models = c('rndForest', 'lr', 'rsvm', 'xgb') # c('rndForest', 'lr', 'lsvm', 'rsvm', 'xgb', 'gbm')
inTrain = createDataPartition(y = groups,
                              times= nsplits,
