...
cl <- makeCluster(6)
res = parSapply(cl, X, nvVSadhd_test, merged, 'pheno ~ y + df$age + I(df$age^2) + df$SEX', 2)
res2 = parSapply(cl, X, outcome_test, merged, 'pheno ~ df$age + I(df$age^2) + df$SEX + y', 4)
stopCluster(cl)
```
```{r}
...
library(parallel)
cl <- makeCluster(6)
res = parSapply(cl, X, nvVSadhd_test, mstruct, 'pheno ~ y + df$age + I(df$age^2) + df$SEX', 2)
res2 = parSapply(cl, X, outcome_test, mstruct, 'pheno ~ df$age + I(df$age^2) + df$SEX + y', 4)
stopCluster(cl)
```
...
write.csv(cbind(t(res), t(res2)), file='~/data/baseline_prediction/descriptives/area.csv', row.names=F)
```
## Thickness
```{r}
source('~/ncr_notebooks/baseline_prediction/src/aux_functions.R')
...
res2 = outcome_test(miq$FSIQ, miq, 'pheno ~ y', 1)

res = rbind(res, sapply(X, nvVSadhd_test, mcpt, 'pheno ~ y + df$age + df$SEX', 2))
...
X = X[keep_me, ]
merged = merged[keep_me, ]
print(sprintf('Neuropsych: %d datapoints, %d phenotypes', nrow(X), ncol(X)))
res = sapply(X, nvVSadhd_test, merged, 'pheno ~ y + df$age + df$SEX', 2)
res2 = sapply(X, outcome_test, merged, 'pheno ~ df$age + df$SEX + y', 3)
