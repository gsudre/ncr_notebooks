...
```
NEED TO REDO ALL THESE RESULTS, BECAUSE THERE WAS A TRASH COLUMN IN THE DATA MATRIX THAT WAS TAKING ALL THE IMPORTANCE. NOT SURE WHY THE RESULTS WERE GOOD, BUT THERE YOU GO. EVERYTHING ABOVE THIS NOTE HAS ALREADY BEEN RE-RUN.

These results were surprisingly encouraging:

```
> print(model_perf)
         rf kernelpls svmRadial       knn     rpart bagEarthGCV LogitBoost       lda        nb
1 0.7945809 0.7851694 0.7246549 0.7530022 0.7504688   0.3470843  0.7932268 0.6444996 0.6470401
> print(model_preds)
                rf kernelpls svmRadial       knn     rpart bagEarthGCV LogitBoost       lda        nb
Accuracy 0.7021277 0.6382979 0.5531915 0.6595745 0.7659574  0.08510638  0.7659574 0.5531915 0.5319149
> print(sprintf('No information rate: Accuracy=%f', max(table(ytrain)/length(ytrain))))
...
> 
```
Several models do a decent job in accuracy, considering that the NIR is 40%. 
...
Another encouraging result using structural voxelwise data:

```
> print(model_perf)
         rf kernelpls svmRadial      knn     rpart bagEarthGCV LogitBoost       lda        nb
1 0.7457274 0.7661444 0.6984507 0.700865 0.6918691   0.4351484  0.7387319 0.6520702 0.6341741
> print(model_preds)
                rf kernelpls svmRadial       knn     rpart bagEarthGCV LogitBoost       lda        nb
Accuracy 0.6808511 0.6382979 0.5957447 0.6808511 0.6808511   0.1489362  0.6046512 0.5319149 0.4680851
> print(sprintf('No information rate: Accuracy=%f', max(table(ytrain)/length(ytrain))))
...
They were weighted: 
7.8904 4.7246 -14.6275 -6.7668 -7.1479 -1.5642 8.0152 -0.3458 0.165 1.0568
The resulting Accuracy is: 0.8554
The fit for each individual model on the Accuracy is: 
      method  Accuracy AccuracySD
          rf 0.7754809 0.08130569
   kernelpls 0.8452569 0.06780754
   svmRadial 0.8401054 0.05442764
         knn 0.6708169 0.08804529
       rpart 0.6671739 0.10296501
 bagEarthGCV 0.7383399 0.09046128
  LogitBoost 0.7534651 0.08417242
         lda 0.7083399 0.09853644
          nb 0.8000988 0.06662096
> model_preds <- lapply(model_list, predict, newdata=Xtest)
...
Accuracy 0.5714286 0.3928571 0.4285714 0.5714286 0.6428571   0.5357143  0.5714286 0.5 0.4285714 0.5357143 0.5357143 0.4642857
> print(sprintf('No information rate: Accuracy=%f', max(table(ytrain)/length(ytrain))))
...
> print(model_preds)
                rf kernelpls svmRadial       knn     rpart bagEarthGCV LogitBoost       lda        nb   greedyE      glmE      gbmE
Accuracy 0.6363636 0.5909091 0.6363636 0.4090909 0.6363636   0.6363636  0.5909091 0.6363636 0.5454545 0.3636364 0.3636364 0.5909091
> print(sprintf('No information rate: Accuracy=%f', max(table(ytrain)/length(ytrain))))
...
Note that I had also tried rpart and MARS. The former couldn't allocate a 400Gb vector, so it crashed. It's a shame becaus eit was getting nice results, but comparable to SVM and rf. MARS died with NANs, but the results were crap anyways before.
...
vdata_area = cbind(struct_data$Mask.ID...Scan, lh_area, rh_area)
rm(lh_area)
rm(rh_area)
gf_fname = '~/data/baseline_prediction/stripped/clinical.csv'
gf = read.csv(gf_fname)
...
rm_me = abs(mstruct$dateX.minus.dateY.months) > 12
mstruct = mstruct[!rm_me, ]
area_base_vdata = merge(mstruct$Mask.ID...Scan, vdata_area, by.x=1, by.y=1, all.y=F, all.x=T)
rm(vdata_area)
```
...
vdata_volume = cbind(lh_volume, rh_volume[,2:ncol(rh_volume)])
rm(lh_volume)
rm(rh_volume)
gf_fname = '~/data/baseline_prediction/stripped/clinical.csv'
gf = read.csv(gf_fname)
gf_base = gf[gf$BASELINE=='BASELINE', ]
my_ids = intersect(gf_base$MRN, vdata_volume$maskid)
mstruct = mergeOnClosestDate(gf_base, struct_data, my_ids)
rm_me = abs(mstruct$dateX.minus.dateY.months) > 12
mstruct = mstruct[!rm_me, ]
volume_base_vdata = merge(mstruct$Mask.ID...Scan, vdata_volume[2:ncol(vdata_volume)], by.x=1, by.y=1, all.y=F, all.x=T)
rm(vdata_volume)
```
